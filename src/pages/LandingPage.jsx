      ...
      {particles.map((_, i) => (
        <motion.div
          ...
          style={{
            ...
            background: i % 2 === 0 ? "#007bff" : "#8a2be2",
          }}
          animate={{ y: [0, -1000] }}
          transition={{
...
function FeatureCard({ Icon, title, desc, delay, accentColor }) {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -5 }}
      className="relative group p-8 rounded-2xl border border-white/5 bg-white/[0.02] backdrop-blur-md transition-all duration-300 hover:border-blue-500/30 hover:shadow-[0_0_30px_rgba(0,123,255,0.1)]"
    >
      <div 
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white selection:bg-blue-500/30">
      ...
      {/* Navbar section stays similar but uses Tailwind for the blur */}
      <nav className="fixed top-0 w-full z-50 px-6 py-4 flex items-center justify-between border-b border-white/10 bg-black/50 backdrop-blur-xl">
        <div className="flex items-center gap-2 group cursor-pointer">
          <div className="w-9 h-9 bg-gradient-to-tr from-blue-600 to-purple-600 rounded-xl flex items-center justify-center shadow-lg group-hover:rotate-12 transition-transform">
            <FiLink className="text-white" />
          </div>
          <span className="font-black text-xl tracking-tighter">Link<span className="text-blue-500">Folio</span></span>
        </div>
        
        <div className="flex items-center gap-4">
          <button onClick={() => navigate("/login")} className="hidden sm:block text-gray-400 hover:text-white transition-colors px-4 py-2 text-sm font-medium">Login</button>
          <button 
            onClick={() => navigate("/signup")}
            className="bg-gradient-to-r from-blue-600 to-purple-600 px-6 py-2.5 rounded-full text-sm font-bold shadow-lg shadow-blue-500/20 hover:shadow-blue-500/40 transition-all hover:scale-105 active:scale-95"
          >
            Join Free
          </button>
        </div>
      ...
      {/* Hero Section */}
      <main className="relative pt-32 pb-20 px-6 max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-12">
        <div className="flex-1 space-y-8 text-center lg:text-left">
          <motion.div 
            initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            className="inline-block px-4 py-1.5 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-400 text-xs font-bold tracking-widest uppercase"
          >
          ...
          <h1 className="text-5xl md:text-7xl font-black leading-[1.1] tracking-tight">
            One Link to <br />
            <span className="bg-gradient-to-r from-blue-500 via-purple-500 to-blue-500 bg-[length:200%_auto] animate-gradient text-transparent bg-clip-text">
              Rule Them All
            </span>
          </h1>

          <p className="text-gray-400 text-lg md:text-xl max-w-2xl mx-auto lg:mx-0 leading-relaxed">
            Beautiful, fast, and customizable bio-link pages for creators. Connect your audience to everything you do in one click.
          </p>

          <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4 pt-4">
            <button className="w-full sm:w-auto px-8 py-4 bg-white text-black font-bold rounded-2xl hover:bg-blue-50 transition-all hover:shadow-[0_0_40px_rgba(255,255,255,0.15)]">
              Get Started Now
            </button>
            <button className="w-full sm:w-auto px-8 py-4 bg-transparent border border-white/10 rounded-2xl font-bold hover:bg-white/5 transition-all">
              View Demo
            </button>
          </div>
        </div>

        {/* Hero Image Component (Aapki HeroIllustration function yahan use hogi) */}
        <div className="flex-1 w-full max-w-lg lg:max-w-none">
           {/* Insert HeroIllustration call here */}
        </div>
      </main>
      ...
      <section id="features" className="py-24 px-6 max-w-7xl mx-auto">
        <div className="text-center mb-16 space-y-4">
          <h2 className="text-3xl md:text-5xl font-black">Built for Creators</h2>
          <p className="text-gray-500">Everything you need to manage your digital presence.</p>
        </div>
      ...
      <footer className="py-12 border-t border-white/5 text-center text-gray-600 text-sm">
        <p>© {new Date().getFullYear()} LinkFolio. Built with ❤️ for the creator economy.</p>
      </footer>
    </div>
  );
}
